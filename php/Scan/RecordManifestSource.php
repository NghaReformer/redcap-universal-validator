...
final class RecordManifestSource
{
    ...
    public function dagsOf(array $ids)
    {
        ...
        if ($this->dagCol === null) {
            if ($this->dataTable === null) return $out;
            $rows = [];
            foreach ($ids as $id) $rows[] = ['id' => (string) $id, 'dag' => null];
            $this->fillGroups($rows);
            foreach ($rows as $r) {
                $out[$r['id']] = ($r['dag'] === null || $r['dag'] === '') ? null : (string) $r['dag'];
            }
            return $out;
        }
        try {
    ...
    private function fillGroups(array &$rows)
    {
        ...
        foreach ($rows as $i => $r) {
            ...
            $rows[$i]['dag'] = ($map[$r['id']] === null || $map[$r['id']] === '')
                             ? null : (string) $map[$r['id']];
        }
    }

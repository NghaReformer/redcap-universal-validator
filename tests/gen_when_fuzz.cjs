...
function add(expr) {
  ...
  c.ok = !!r.ok;
  if (r.ok) {
    c.js = W.evaluate(r.ast, v, W.BLANK_PASSES);
    ...
    c.jsInert = W.evaluate(r.ast, v, W.BLANK_INERT);
...
fs.writeFileSync(out, JSON.stringify({
  note: 'GENERATED by tests/gen_when_fuzz.cjs — do not hand-edit. Seeded conditions (valid and mutated) with the JS twin\'s verdicts frozen in BOTH blank-operand roles (js = @UVASSERT, jsInert = when-gate); tests/when_fuzz_php.php makes php/Logic.php recompute them and fails on any disagreement.',
  seeds: SEEDS,
  cases: cases,
}, null, 1) + '\n');

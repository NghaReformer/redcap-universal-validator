...
function add(expr) {
  ...
  c.ok = !!r.ok;
  if (r.ok) {
    /* js / jsInert are the EXACT verdicts ("caseSensitive": true). The flag is
       passed explicitly: the evaluator's own default is case-insensitive. */
    c.js = W.evaluate(r.ast, v, W.BLANK_PASSES, true);
    /* BOTH polarities (CRIT-01). `js` is the @UVASSERT role and keeps its name
       and its meaning; `jsInert` is the "when" gate role. A blank operand in an
       ordered comparison is the ONE input where the two roles disagree, and the
       corpus is full of blanks ('' is in both VALS and LITS), so this is where
       the two runtimes are most likely to drift apart unnoticed. */
    c.jsInert = W.evaluate(r.ast, v, W.BLANK_INERT, true);
    /* Both roles again, case-insensitive: the module default for every rule
       condition unless its rule sets "caseSensitive": true. */
    c.jsCi = W.evaluate(r.ast, v, W.BLANK_PASSES, false);
    c.jsInertCi = W.evaluate(r.ast, v, W.BLANK_INERT, false);
...
/* CASE batch, appended so every case above keeps its seed stream. The main
   pools hold few letters, so on their own they barely separate js from jsCi.
   These pools are mostly text differing only in case, plus non-ASCII values
   (literals must be ASCII, field values need not be) whose accented letters
   must NOT fold, and a few numbers and blanks so the fold meets the other
   domains. Swapped in place: VALS/LITS are read through pick() at call time. */
const CASE_SEED = 20260914;
const CASE_VALS = ['yes', 'Yes', 'YES', 'no', 'NO', 'abc', 'ABC', 'Abc', 'b', 'B', 'a1', 'A1',
                   'École', 'école', 'CAFé', 'café', 'Z', '[', '_', '', ' yes ', '2', '10'];
const CASE_LITS = ['yes', 'YES', 'No', 'abc', 'ABC', 'b', 'B', 'A1', 'z', 'Z', '_', '', '2'];
{
  const keepVals = VALS.splice(0, VALS.length, ...CASE_VALS);
  const keepLits = LITS.splice(0, LITS.length, ...CASE_LITS);
  srand(CASE_SEED);
  for (let i = 0; i < 2000; i++) add(expr(1 + int(2)));
  VALS.splice(0, VALS.length, ...keepVals);
  LITS.splice(0, LITS.length, ...keepLits);
}

const out = path.join(__dirname, 'when_fuzz.json');
fs.writeFileSync(out, JSON.stringify({
  note: 'GENERATED by tests/gen_when_fuzz.cjs — do not hand-edit. Seeded conditions (valid and mutated) with the JS twin\'s verdicts frozen in BOTH blank-operand roles (js = @UVASSERT, jsInert = when-gate) plus both roles case-insensitive (jsCi, jsInertCi); tests/when_fuzz_php.php makes php/Logic.php recompute them and fails on any disagreement.',
  seeds: SEEDS,
  caseSeed: CASE_SEED,
  cases: cases,
}, null, 1) + '\n');
